...
def plot_set(x_range,y_range,sites,grid,pointsize,pes_type):
    N = 100
    ...
    ax.plot(locs[:,0],locs[:,1],'bs',markersize=pointsize)
    plt.show()    
    
def plot_edges(x_range,y_range,sites,avg_edgecount,grid,pointsize,pes_type):
    N = 100
    x_vec = np.linspace(x_range[0], x_range[1], N)
    y_vec = np.linspace(y_range[0], y_range[1], N)
    energy = np.zeros((N, N))
    ...
    ax.tick_params(axis='both',labelsize=12)
    locs = np.asarray(locs)
    ax.scatter(locs[:,0],locs[:,1],c=locs[:,2],marker='s',s=pointsize)
    plt.show()

def plot_set_cross(x_range,y_range,basin_A_sites,set_sites,first_crosses,grid,pointsize,pes_type):
    N = 100
    x_vec = np.linspace(x_range[0], x_range[1], N)
    y_vec = np.linspace(y_range[0], y_range[1], N)
    energy = np.zeros((N, N))
    ...
    for site in set_sites: 
        ...
        locs.append(cords)      
    locs = np.asarray(locs)
    ax.plot(locs[:,0],locs[:,1],'gs',markersize=pointsize)
    locs = []
    ...
    ax.plot(first_crosses[:,0],first_crosses[:,1],'ro',markersize=pointsize)
    plt.show()
